...
// Sirve el sello directamente en /sello.png, sin importar el nombre exacto
// del archivo en el repositorio (por ejemplo, si GitHub lo guardó como
// "assets:sello-te-lo-dije.png" en la raíz en vez de dentro de una carpeta).
// Así Lob siempre puede alcanzarlo en:  https://tu-backend.onrender.com/sello.png
const STAMP_FILENAME = process.env.STAMP_FILENAME || 'assets:sello-te-lo-dije.png';
app.get('/sello.png', (req, res) => {
  res.sendFile(path.join(__dirname, STAMP_FILENAME), (err) => {
    if (err) {
      console.error('No se pudo servir el sello:', err.message);
      res.status(404).send('Imagen del sello no encontrada. Revisa STAMP_FILENAME en .env');
    }
  });
});
